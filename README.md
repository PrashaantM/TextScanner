...
- Select multiple words at once in either image-based view, by shift-clicking and dragging a selection box on a mouse, or by pressing and holding empty space and dragging on touch, since a phone has no shift key and a plain finger drag belongs to scrolling the page
- In **Full image**, tap a word - or the photo itself - to select it, then drag from the small handle that appears to move it, or its corner handle to resize it: for moving and resizing the text and the image itself, freely and independently, with Undo and Redo for every move and resize
...
- Responsive layout in one signature dark colour scheme, tuned so text stays readable over your own photographs rather than only over the app's own surfaces
...
3. Read the result in **Text**, reposition-edit each word over a plain background in **Image format**, or work directly on the photo in **Full image**. In any of the two image-based views, shift-click or press-and-hold-and-drag to select several words, then copy or download just that selection, as text or as a PNG.
4. In **Full image**, tap a word - or the photo itself - to select it, then drag its handle to move it or its corner handle to resize it. Undo and Redo step back and forward through those changes.
...
style.css             Styling: one colour scheme, every colour a custom
                      property on :root, and the --motion-* tokens a single
